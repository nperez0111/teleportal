...
To see real-time sync in action, open a second terminal and run the client again. Both clients connect to the same document, so edits from one appear in the other. If you modify the client script to insert different text, you'll see both clients converge to the same state -- that's the CRDT at work.
