...
<Aside type="tip">
  Route WebSocket upgrades and HTTP/SSE requests under the same path prefix (e.g. `/api/*`) so
  both transports reach the same Durable Object instance. A WebSocket upgrade is a plain `GET /`,
  so if you also configure Workers `[assets]`, keep the API prefix distinct from your static
  asset paths or the upgrade will never reach the Worker.
</Aside>
