...
The `messageTypeBreakdown` is keyed by wire message type (`doc`, `awareness`, `rpc`, `ack`) -- presence, milestone, and file operations are carried over `rpc`, not distinct message types.
