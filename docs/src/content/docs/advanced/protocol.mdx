...
**Note**: Presence, milestones, file transfers, attribution, and key distribution are **not** dedicated wire message types. They are all RPC methods carried by the RPC message (type `0x04`) — e.g. `presenceAnnounce`/`presenceJoin`, `milestoneList`/`milestoneCreate`, `fileUpload`/`fileDownload`, `attributionActivity`/`attributionGet`. Only the four wire types above exist at the protocol level (type `0x03`, formerly presence, is deleted).
...
Presence — which clients are in a document, distinct from Y.js awareness (cursor/selection state) — is **not** a dedicated wire type. Wire type `0x03` is deleted; presence is an RPC protocol (`teleportal/protocols/presence`) built on request-response methods (`presenceAnnounce`/`presenceUnannounce`) and server-authored pushes (`presenceJoin`/`presenceLeave`/`presenceRoster`) carried as RPC messages (type `0x04`).
...
A response with **no** `originalRequestId` (flag byte `0`) is a **push**: an unsolicited server-authored notification correlated to no request (e.g. `presenceJoin`). Push methods declare per-method delivery QoS (`durability`, `replicate`, `ack`, `dedupe`) via `definePush` in `teleportal/rpc`.
