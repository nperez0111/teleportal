...
**Note**: File transfers (upload, download, file parts, file auth) are implemented as RPC messages (type 0x04) using the `fileUpload` and `fileDownload` RPC methods with streaming support.
...
| ------------------ | ------ | ------------------------------------------------------------------------------------------------------------- |
| Sync Step 1        | `0x00` | State Vector (varint array)                                                                                   |
| Sync Step 2        | `0x01` | Y.js Update (varint array)                                                                                    |
| Doc Update         | `0x02` | Y.js Update (varint array)                                                                                    |
| Sync Done          | `0x03` | (no payload)                                                                                                  |
| Auth Message       | `0x04` | Permission (1 byte: `0x00`=denied, `0x01`=allowed) + Reason (varint string)                                   |
| Mile List Req      | `0x05` | SnapshotIds count (varint) + SnapshotIds array (varint strings)                                               |
| Mile List Resp     | `0x06` | Count (varint) + [Id + Name + DocId + CreatedAt + DeletedAt? + LifecycleState? + ExpiresAt? + CreatedBy] \* N |
| Mile Snapshot Req  | `0x07` | MilestoneId (varint string)                                                                                   |
| Mile Snapshot Resp | `0x08` | MilestoneId (varint string) + Snapshot (varint array)                                                         |
| Mile Create Req    | `0x09` | HasName (1 byte) + Name (varint string, optional) + Snapshot (varint array)                                   |
| Mile Create Resp   | `0x0A` | Id + Name + DocId + CreatedAt + CreatedBy (Type + Id)                                                         |
| Mile Update Name   | `0x0B` | MilestoneId (varint string) + Name (varint string)                                                            |
| Mile Update Resp   | `0x0C` | Id + Name + DocId + CreatedAt + CreatedBy (Type + Id)                                                         |
| Mile Auth Msg      | `0x0D` | Permission (1 byte: `0x00`=denied, `0x01`=allowed) + Reason (varint string)                                   |
| Mile SoftDel Req   | `0x0E` | MilestoneId (varint string)                                                                                   |
| Mile SoftDel Resp  | `0x0F` | MilestoneId (varint string)                                                                                   |
| Mile Restore Req   | `0x10` | MilestoneId (varint string)                                                                                   |
| Mile Restore Resp  | `0x11` | MilestoneId (varint string)                                                                                   |
...
| Msg Type | Code   | Payload                                              |
| -------- | ------ | ---------------------------------------------------- |
| ACK      | `0x02` | MessageId (varint array) - Base64-decoded message ID |
...
## File Messages (via RPC)

File messages handle file uploads and downloads with chunking and Merkle tree verification for integrity. Files are transferred as RPC messages (type 0x04) using the `fileUpload` and `fileDownload` RPC methods with streaming support. Files are transferred in chunks with cryptographic proofs to ensure data integrity.

### File Message Structure

| Msg Type          | Code   | Payload                                                                                                                                                                                         |
| ----------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| File Download     | `0x00` | FileId (varint string)                                                                                                                                                                          |
| File Upload       | `0x01` | Encrypted (1 byte) + FileId (varint string) + Filename (string) + Size (varint) + MimeType (string) + LastModified (varint)                                                                     |
| File Part         | `0x02` | FileId (varint string) + ChunkIndex (varint) + ChunkData (varint array) + MerkleProofLength (varint) + MerkleProof (array) + TotalChunks (varint) + BytesUploaded (varint) + Encrypted (1 byte) |
| File Auth Message | `0x03` | Permission (1 byte) + FileId (varint string) + StatusCode (varint) + HasReason (1 byte) + Reason (varint string, optional)                                                                      |
...
1. **Client sends**: Chunk data (up to 1MB) + Merkle proof (sibling hashes) + Chunk index
...
sequenceDiagram
    ...
    Client->>Server: File Upload (metadata: uploadId, filename, size, etc.)
    Note over Server: Creates upload session
    Client->>Server: File Part (chunk 0 + merkle proof)
    Note over Server: Verifies chunk, stores
    Client->>Server: File Part (chunk 1 + merkle proof)
    Note over Server: Verifies chunk, stores
    Note over Client,Server: ... (more chunks)
    Client->>Server: File Part (final chunk + merkle proof)
    Note over Server: Verifies all chunks,<br/>reconstructs merkle tree,<br/>stores file, removes session
    Server->>Client: File Auth Message (optional: returns contentId/fileId)
```
...
sequenceDiagram
    ...
    Client->>Server: File Download (fileId: merkle root hash)
    Note over Server: Looks up file by fileId/contentId
    Server->>Client: File Part (chunk 0 + merkle proof)
    Note over Client: Verifies chunk
    Server->>Client: File Part (chunk 1 + merkle proof)
    Note over Client: Verifies chunk
    Note over Client,Server: ... (more chunks)
    Server->>Client: File Part (final chunk + merkle proof)
    Note over Client: Verifies all chunks,<br/>reconstructs file
    Server->>Client: File Auth Message (optional: error if file not found)
```
