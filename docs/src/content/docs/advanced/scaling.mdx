...
- `RedisPubSub` is **durable by default**: document updates are persisted to a per-topic Redis Stream (`XADD`), and a node that briefly disconnects resumes its `XREAD` from the last-read id and catches up on everything it missed — so it no longer silently serves stale state after a blip. Ephemeral traffic (presence, awareness, acks) still uses plain fire-and-forget `PUBLISH` so it doesn't consume stream retention.
