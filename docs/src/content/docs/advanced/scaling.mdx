...
- `RedisPubSub` is **durable by default**: document updates are persisted to a per-topic Redis Stream (`XADD`), and a node that briefly disconnects resumes its `XREAD` from the last-read id and catches up on everything it missed — so it no longer silently serves stale state after a blip. Ephemeral traffic (awareness, acks, presence RPC pushes) still uses plain fire-and-forget `PUBLISH` so it doesn't consume stream retention.
- Retention is bounded by `stream.maxLen` (approximate `MAXLEN`, default 10_000) and an optional rolling `stream.ttlMs`. If a node is disconnected long enough that its resume position is trimmed out of retention, the backend reports a gap and the session heals by re-syncing its local clients from storage (see [PubSub durability](/guides/pub-sub/)).
- Set `stream: { isDurableTopic: () => false }` to fully restore the old fire-and-forget behavior with zero stream overhead.
- Fan-out to many subscribers on a single Redis instance can add latency under high load.
- `RedisPubSub` uses four connections per instance (plain publish/subscribe pair, one command connection, one dedicated blocking `XREAD` connection).
...
- `NatsPubSub` uses core NATS and is fire-and-forget (like plain Redis PUBLISH). For durability, import `NatsJetStreamPubSub` from `teleportal/transports/nats/jetstream` — it persists document updates to a JetStream stream and catches up after a disconnect, mirroring the durable `RedisPubSub`. It requires a JetStream-enabled server (`nats-server -js`) and the optional `@nats-io/jetstream` dependency; core-NATS users of `NatsPubSub` never pull it in. See [PubSub durability](/guides/pub-sub/).
