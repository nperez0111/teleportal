...
<Aside type="caution">
  This connection method has some slight overhead compared to WebSocket, in that, because it is run
  over HTTP, it does not support binary data & instead must use base64-encoded strings for all data.
</Aside>
