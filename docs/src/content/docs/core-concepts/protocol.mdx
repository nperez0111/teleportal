...
The Teleportal protocol is a binary protocol built on top of Y.js. It defines four wire message types — document sync, awareness, ack, and RPC — where higher-level features like presence, file transfers, and milestones ride on top of RPC. This document explains the design decisions and rationale behind the protocol's architecture.
...
- **Milestones**: `milestone.list`, `milestone.create`, `milestone.get`, etc.
- **Files**: `file.upload`, `file.download`
- **Attribution**: `attribution.activity`, `attribution.get`, etc.
...
- **Header**: magic (`YJS`), version, document name, encryption flag, best-effort flag, and the 1-byte message type
