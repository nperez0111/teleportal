...
Attribution RPC methods (`attribution.activity`, `attribution.get`) are covered by the server's global `checkPermission` hook. Use the `rpcMethod` field to apply method-level authorization:
