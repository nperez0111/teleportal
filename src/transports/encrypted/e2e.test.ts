...
function createServerClient(
  id: string,
  onMessage: (msg: Message<Ctx>) => void,
): Client<Ctx> {
...
describe("encrypted sync e2e: two clients via server", () => {
  ...
  async function applyBroadcastedUpdates(
    encClient: EncryptionClient,
    inbox: Message<Ctx>[],
  ) {
    for (const msg of inbox) {
      ...
      if (msg.payload.type === "update") {
        await encClient.handleUpdate(
          msg.payload.update as unknown as EncryptedUpdatePayload,
        );
      }
    }
  }
  ...
  it("client A writes, client B connects and receives the document", async () => {
    ...
    const serverClientA = createServerClient("client-a", (msg) =>
      inboxA.push(msg),
    );
    ...
    const serverClientB = createServerClient("client-b", (msg) =>
      inboxB.push(msg),
    );
  ...
  it("concurrent edits from both clients converge", async () => {
    ...
    expect(ydocA.getText("body").toString()).toBe(
      ydocB.getText("body").toString(),
    );
...
describe("encrypted sync: snapshot ID mismatch", () => {
  ...
  it("updates propagate even when receiver has a newer snapshot", async () => {
    ...
    const snapshotPayload = await clientB.encryptUpdate(
      Y.encodeStateAsUpdateV2(ydocB) as Update,
    );
...
describe("encrypted sync e2e: full WebSocket transport", () => {
  ...
  function waitForContent(
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(
          new Error(
            `Timed out waiting for content. Current: "${ydoc.getText(field).toString()}"`,
          ),
        );
      }, timeoutMs);
      const cleanup = () => {
  ...
  it("two unencrypted clients sync a document over WebSocket", async () => {
    ...
    const final = await waitForContent(
      pA.doc,
      "body",
      (t) => t === "hello from A and B",
    );
  ...
  it("encrypted bidirectional sync: both clients exchange live updates", async () => {
    ...
    const textA = await waitForContent(
      pA.doc,
      "body",
      (t) => t === "hello world",
    );
    ...
    const textB = await waitForContent(
      pB.doc,
      "body",
      (t) => t === "hello world!",
    );
  ...
  it("single connection handles multiple documents", async () => {
    ...
    const text1 = await waitForContent(
      p1b.doc,
      "body",
      (t) => t === "doc one",
    );
    const text2 = await waitForContent(
      p2b.doc,
      "body",
      (t) => t === "doc two",
    );
