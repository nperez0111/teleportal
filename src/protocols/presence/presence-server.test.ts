...
function announceMessage(
  method: "presenceAnnounce" | "presenceUnannounce",
  awarenessId: number,
  userId: string,
...
describe("presence protocol server", () => {
  ...
  it("replies to an announce with the roster, including the announcer's own entry", async () => {
    ...
    await session.apply(announceMessage("presenceAnnounce", 11, "user-a"), client as any);

    const rosters = client.pushes("presenceRoster") as Array<{ clients: PresenceEntry[] }>;
  ...
  it("notifies already-announced peers of a newcomer, but not un-announced connections", async () => {
    ...
    await session.apply(announceMessage("presenceAnnounce", 11, "user-a"), announced as any);
    await session.apply(announceMessage("presenceAnnounce", 22, "user-b"), newcomer as any);

    const joins = announced.pushes("presenceJoin") as PresenceEntry[];
    expect(joins).toHaveLength(1);
    expect(joins[0].awarenessId).toBe(22);
    expect(silent.pushes("presenceJoin")).toHaveLength(0);
    // The newcomer's roster carries both entries.
    const rosters = newcomer.pushes("presenceRoster") as Array<{ clients: PresenceEntry[] }>;
  ...
  it("broadcasts an unannounce as a leave to everyone, including the sender's connection", async () => {
    ...
    await session.apply(announceMessage("presenceAnnounce", 11, "user-a"), clientA as any);
    await session.apply(announceMessage("presenceAnnounce", 22, "user-b"), clientB as any);

    await session.apply(announceMessage("presenceUnannounce", 11, "user-a"), clientA as any);

    const leavesAtB = clientB.pushes("presenceLeave") as PresenceEntry[];
    expect(leavesAtB.map((l) => l.awarenessId)).toEqual([11]);
    // Sibling tabs on the sender's own SharedWorker connection need it too.
    const leavesAtA = clientA.pushes("presenceLeave") as PresenceEntry[];
  ...
  it("broadcasts a leave when a client disconnects (client-leave event)", async () => {
    ...
    await waitFor(() => clientB.pushes("presenceLeave").length === 1);
    const leaves = clientB.pushes("presenceLeave") as PresenceEntry[];
  ...
  it("does not create a ghost peer when the client leaves during an async getPresenceData", async () => {
    ...
    await session.apply(announceMessage("presenceAnnounce", 22, "user-b"), observer as any);

    // Start the leaver's announce; it suspends inside the gated getPresenceData.
    const announcePromise = session.apply(
      announceMessage("presenceAnnounce", 11, "user-a"),
      leaver as any,
    );
    ...
    expect(observer.pushes("presenceJoin")).toHaveLength(0);
    await runPresenceMaintenance(registry, session);
    const rosters = observer.pushes("presenceRoster") as Array<{ clients: PresenceEntry[] }>;
  ...
  it("transfers an awarenessId re-announced from a new connection without a leave", async () => {
    ...
    session.addClient(observer as any);
    await session.apply(announceMessage("presenceAnnounce", 33, "user-x"), oldConn as any);
    await session.apply(announceMessage("presenceAnnounce", 44, "user-o"), observer as any);

    await session.apply(announceMessage("presenceAnnounce", 33, "user-x"), newConn as any);
    // The old connection's eventual disconnect must not clobber the live presence.
    session.removeClient("client-old");
    await new Promise((resolve) => setTimeout(resolve, 5));

    expect(observer.pushes("presenceLeave")).toHaveLength(0);
  });
  ...
  it("ignores forged server-authored pushes from clients", async () => {
    ...
    await session.apply(announceMessage("presenceAnnounce", 22, "user-b"), victimPeer as any);

    await session.apply(
      forgedPush("presenceLeave", {
        awarenessId: 22,
    ...
    expect(victimPeer.pushes("presenceLeave")).toHaveLength(0);
  });
  ...
  describe("cross-node", () => {
    it("propagates joins to other nodes and reconciles rosters", async () => {
      ...
      await nodeB.session.apply(
        announceMessage("presenceAnnounce", 22, "user-b"),
        remoteClient as any,
      );
      ...
      await waitFor(() => remoteClient.pushes("presenceJoin").length === 1);
      const join = (remoteClient.pushes("presenceJoin") as PresenceEntry[])[0];
      ...
      const rosters = remoteClient.pushes("presenceRoster") as Array<{
    ...
    it("re-learns peers after a replication gap via a roster request", async () => {
      ...
      await runPresenceMaintenance(nodeB.registry, nodeB.session);
      await waitFor(() => remoteClient.pushes("presenceLeave").length === 1);
      ...
      await waitFor(() => remoteClient.pushes("presenceJoin").length === 2);
      const rejoin = (remoteClient.pushes("presenceJoin") as PresenceEntry[])[1];
    ...
    it("keeps presence alive when a client reconnects to another node before its old connection dies", async () => {
      ...
      await nodeA.session.apply(
        announceMessage("presenceAnnounce", 1, "user-oa"),
        observerA as any,
      );
      await nodeB.session.apply(
        announceMessage("presenceAnnounce", 2, "user-ob"),
        observerB as any,
      );
      // Client X (awarenessId 33) is on node A...
      await nodeA.session.apply(announceMessage("presenceAnnounce", 33, "user-x"), oldConn as any);
      await waitFor(() =>
        (observerB.pushes("presenceJoin") as PresenceEntry[]).some((j) => j.awarenessId === 33),
      );

      // ...then reconnects to node B while the old connection lingers on A.
      await nodeB.session.apply(announceMessage("presenceAnnounce", 33, "user-x"), newConn as any);
      await waitFor(() => {
        const rosters = observerB.pushes("presenceRoster");
        return (
          rosters.length > 0 || (observerA.pushes("presenceJoin") as PresenceEntry[]).length >= 1
        );
      });
      await new Promise((resolve) => setTimeout(resolve, 5));
      ...
      expect(
        (observerA.pushes("presenceLeave") as PresenceEntry[]).filter((l) => l.awarenessId === 33),
      ).toHaveLength(0);
      expect(
        (observerB.pushes("presenceLeave") as PresenceEntry[]).filter((l) => l.awarenessId === 33),
      ).toHaveLength(0);
    ...
    it("delivers a re-join within the replication dedupe window (announce → unannounce → re-announce)", async () => {
      ...
      await waitFor(() => remoteClient.pushes("presenceJoin").length === 1);
      await nodeA.session.apply(
        announceMessage("presenceUnannounce", 11, "user-a"),
        localClient as any,
      );
      await waitFor(() => remoteClient.pushes("presenceLeave").length === 1);
    ...
    it("coalesces roster refreshes within the configured window", async () => {
      ...
      await pubSub.subscribe("document/test-doc", (binary, sourceId) => {
        ...
        if (decoded.type === "rpc" && decoded.rpcMethod === "presenceRosterRequest") {
    ...
    it("suppresses repeated roster answers within the configured window", async () => {
      ...
      await pubSub.subscribe("document/test-doc", (binary, sourceId) => {
        const decoded = decodeMessage(binary);
        if (decoded.type === "rpc" && decoded.rpcMethod === "presenceRoster") {
    ...
    it("heals a lost join via the periodic node roster (dedupe-exempt) and expires crashed nodes", async () => {
      ...
      await runPresenceMaintenance(nodeB.registry, nodeB.session);
      await waitFor(() => remoteClient.pushes("presenceLeave").length === 1);
      const leave = (remoteClient.pushes("presenceLeave") as PresenceEntry[])[0];
