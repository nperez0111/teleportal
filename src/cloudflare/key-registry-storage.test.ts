...
import { DurableObjectKeyRegistryStorage } from "./key-registry-storage";

function wrap(s: string): Uint8Array {
  return new TextEncoder().encode(s);
}

describe("DurableObjectKeyRegistryStorage", () => {
  let storage: DurableObjectKeyRegistryStorage;

  beforeEach(() => {
    storage = new DurableObjectKeyRegistryStorage(new FakeDOStorage(), { keyPrefix: "keys" });
  });
  ...
  it("should use default keyPrefix when none provided", async () => {
    const s = new DurableObjectKeyRegistryStorage(new FakeDOStorage());
  ...
  describe("transaction", () => {
    it("should execute callback and return its result", async () => {
      const result = await storage.transaction("doc-1", async () => "done");
  ...
  describe("persistence", () => {
    it("should persist keys and generation across storage instances", async () => {
      const fake = new FakeDOStorage();
      const s1 = new DurableObjectKeyRegistryStorage(fake, { keyPrefix: "keys" });
      await s1.set("doc-1", [{ userId: "alice", wrappedKey: wrap("v0") }]);
      await s1.rotate("doc-1", [{ userId: "alice", wrappedKey: wrap("v1") }], 0);

      const s2 = new DurableObjectKeyRegistryStorage(fake, { keyPrefix: "keys" });
      const result = await s2.get("doc-1", "alice");
      expect(result!.wrappedKey).toEqual(wrap("v1"));
      expect((await s2.getMeta("doc-1")).generation).toBe(1);
    });
  });
});
